...
public class WorkOrderService {
    ...
    /**
     * Crea una nueva orden de trabajo a partir de los datos recibidos en el DTO de
     * solicitud, guardándola en la base de datos y devolviendo un DTO de respuesta
     * con los datos de la orden creada.
     * 
     * @param request DTO con los datos necesarios para crear la orden de trabajo,
     *                validado automáticamente por Spring
     * @return DTO con los datos de la orden de trabajo creada, incluyendo el número
     *         de orden generado automáticamente
     */
    @Transactional
    ...
    /**
     * Lista todas las órdenes de trabajo existentes en la base de datos,
     * transformándolas a DTOs de respuesta.
     * 
    ...
    /**
     * Obtiene una orden de trabajo por su ID, lanzando una excepción si no se
     * encuentra.
     * 
    ...
    /**
     * Obtiene una orden de trabajo por su número de orden, lanzando una excepción
     * si no se encuentra.
     * 
     * @param numeroOrden El número de orden a buscar
     * @return WorkOrderResponseDTO con los datos de la orden de trabajo encontrada,
     *         o una excepción si no se encuentra ninguna orden con ese número
     */
    @Transactional(readOnly = true)
    ...
    /**
     * Lista las órdenes de trabajo que coinciden con la placa del vehículo
     * proporcionada, transformándolas a DTOs de respuesta. La búsqueda es
     * insensible a mayúsculas y espacios.
     * 
     * @param placa La placa del vehículo a buscar
     * @return Lista de WorkOrderResponseDTO con los datos de las órdenes de trabajo
     *         que coinciden con la placa, o una lista vacía si no se encuentran
     *         coincidencias
     */
    @Transactional(readOnly = true)
    ...
    /**
     * Lista las órdenes de trabajo que coinciden con la identificación del cliente
     * proporcionada, transformándolas a DTOs de respuesta. La búsqueda es
     * insensible a mayúsculas y espacios.
     * 
     * @param identificacion La identificación del cliente a buscar
     * @return Lista de WorkOrderResponseDTO con los datos de las órdenes de trabajo
     *         que coinciden con la identificación, o una lista vacía si no se
    ...
    /**
     * Lista las órdenes de trabajo que coinciden con el estado proporcionado,
     * transformándolas a DTOs de respuesta.
     * 
     * @param stateorder El estado de la orden a buscar (RECIBIDO, EN_DIAGNOSTICO,
     *                   EN_REPARACION, LISTO, ENTREGADO, CANCELADO)
     * @return Lista de WorkOrderResponseDTO con los datos de las órdenes que
     *         coinciden con el estado, o lista vacía si no hay coincidencias
     */
    @Transactional(readOnly = true)
    ...
    /**
     * Actualiza los datos de una orden de trabajo antes de iniciar el servicio (HU
     * 3.3).
     ...
     * @param newState El nuevo estado a asignar
     * @return WorkOrderResponseDTO con el estado actualizado
     */
    @Transactional
    public WorkOrderResponseDTO changeState(Long id, WorkOrder.StateOrder newState) {
        WorkOrder orden = ordenTrabajoRepository.findById(id)
                .orElseThrow(() -> new RuntimeException("Orden no encontrada con id: " + id));

        if (orden.getStateorder() == WorkOrder.StateOrder.CANCELADO) {
            throw new IllegalStateException("No se puede cambiar el estado de una orden cancelada.");
        }
        if (orden.getStateorder() == WorkOrder.StateOrder.ENTREGADO) {
            throw new IllegalStateException("No se puede cambiar el estado de una orden ya entregada.");
        }

        orden.setStateorder(newState);
        WorkOrder guardada = ordenTrabajoRepository.save(orden);
        return toResponse(guardada);
    }

    /**
     * Genera un número de orden único en formato "OT-AAAA-NNNN", donde "AAAA" es el
     * año actual y "NNNN" es un número secuencial que se reinicia cada año. El
     * método verifica que el número generado no exista ya en la base de datos,
     * incrementando el número secuencial hasta encontrar uno disponible.
     * 
    ...
    private WorkOrderResponseDTO toResponse(WorkOrder o) {
        ...
        dto.setAssignedMechanicName(o.getAssignedMechanicName());
        dto.setAssignedMechanicDocument(o.getAssignedMechanicId());
        return dto;
    }
}
